<?php

namespace superwoo_coupon\superwoo_coupon_Coupon\Frontend;

use superwoo_coupon\superwoo_coupon_Coupon\Frontend\Helpers\Apply;
use superwoo_coupon\superwoo_coupon_Coupon\Frontend\Helpers\Validator;

/**
 * Class superwoo_coupon_auto
 * control auto coupon system
 */
class superwoo_coupon_auto
{

	public function __construct()
	{
		if (is_admin()) {
			return;
		}
		add_action('wp_loaded', [$this, "superwoo_coupon_auto_coupon_on_cart"]);
		add_action('woocommerce_cart_calculate_fees', [$this, "superwoo_coupon_auto_coupon_on_cart"]);
		add_filter("woocommerce_product_get_price", [$this, "superwoo_coupon_change_price"], 10, 2);
		add_filter("woocommerce_product_variation_get_price", [$this, "superwoo_coupon_variable_change_price"], 10, 2);
	}

	/**
	 * woocommerce auto coupon function
	 **/
	public function superwoo_coupon_auto_coupon_on_cart()
	{
		do_action('superwoo_coupon_before_wp_loaded');
		$this->superwoo_coupon_first_order();
		$this->superwoo_coupon_auto_coupon();
		do_action('superwoo_coupon_after_wp_loaded');
	}

	/**
	 * superwoo_coupon Auto Coupon
	 **/
	public function superwoo_coupon_auto_coupon()
	{
		$args       = [
			"post_type"   => "woocoupon",
			'post_status' => 'publish'
		];
		$posts      = get_posts($args);
		$woocoupons = $this->superwoo_coupon_filter_woocoupn($posts);
		$first_coupon          = get_option("superwoo_coupon_first_time_purchase_coupon");
		if (count($woocoupons) == 0) {
			if ($first_coupon == 0) {
				WC()->session->__unset("superwoo_coupon_product_coupon");
			}
		}
		foreach ($woocoupons as $woocoupon) {
			$superwoo_coupon_main = get_post_meta($woocoupon->ID, "superwoo_coupon_coupon_main", true);
			if ($superwoo_coupon_main["type"] != "product") {
				if ($first_coupon == 0) {
					WC()->session->__unset("superwoo_coupon_product_coupon");
				}
			}
			$validate = Validator::check(null, null, $woocoupon->ID);
			if ($validate) {
				$apply                 = new Apply;
				$res_data              = $apply->apply_discount($woocoupon->ID);
				$superwoo_coupon_woo_setting_multi = get_option("superwoo_coupon_woo_setting_multi");
				if ($res_data) {
					$cart = WC()->cart;
					if ($superwoo_coupon_woo_setting_multi == "yes") {
						$cart->add_fee($res_data["label"], -$res_data["amount"]);
					} else {
						$cart->add_fee($res_data["label"], -$res_data["amount"]);
						break;
					}
				}
			}
		}
	}

	/**
	 * filter woocoupon
	 * @return filter_posts
	 **/
	public function superwoo_coupon_filter_woocoupn($posts)
	{
		$filter_posts = [];

		$superwoo_coupon_coupons = [];
		$args        = array(
			'posts_per_page' => -1,
			'order'          => 'asc',
			'post_type'      => 'shop_coupon',
			'post_status'    => 'publish',
		);
		$coupons     = get_posts($args);
		$first_coupon = get_option("superwoo_coupon_first_time_purchase_coupon");

		if (count($coupons) != 0) {
			foreach ($coupons as $coupon) {
				$post_meta = get_post_meta($coupon->ID, "superwoo_coupon_coupon_panel", true);
				if ($post_meta != '') {
					if (!empty($post_meta["list_id"]) || $post_meta["list_id"] != '') {
						array_push($superwoo_coupon_coupons, $post_meta["list_id"]);
					}
				}
				
			}
			foreach ($posts as $post) {
				if (!in_array($post->ID, $superwoo_coupon_coupons) && $first_coupon != $post->ID) {
					array_push($filter_posts, $post);
				}
			}
		} else {
			foreach ($posts as $post) {
				if ($first_coupon != $post->ID) {
					array_push($filter_posts, $post);
				}
			}
		}

		return $filter_posts;
	}

	/**
	 * control first purchase features
	 **/
	public function superwoo_coupon_first_order()
	{
		if (!is_user_logged_in()) {
			return;
		}

		$user    = wp_get_current_user();
		$user_id = $user->ID;
		$coupon  = get_option("superwoo_coupon_first_time_purchase_coupon");
		if ($coupon == 0) {
			$session_data = WC()->session->get("superwoo_coupon_product_coupon");
			if ($session_data && $session_data["first_coupon"] == "yes") {
				WC()->session->__unset("superwoo_coupon_product_coupon");
				return;
			}
			return;
		}
		$args   = array(
			'customer_id' => $user_id
		);
		$orders = wc_get_orders($args);
		if (count($orders) == 0) {
			$validate = Validator::check(null, null, $coupon);
			if ($validate) {
				$apply    = new Apply;
				$res_data = $apply->apply_discount($coupon);
				if ($res_data) {
					$cart = WC()->cart;
					$cart->add_fee($res_data["label"], -$res_data["amount"]);
				}
			}
		}
	}

	/**
	 * woocommerce product change price
	 **/
	public function superwoo_coupon_change_price($price, $product)
	{
		$data = WC()->session->get("superwoo_coupon_product_coupon");
		if ($data) {
			if ($data["first_coupon"] === "yes") {
				$coupon          = get_option("superwoo_coupon_first_time_purchase_coupon");
				if ($coupon == 0) {
					return $price;
				}
				$superwoo_coupon_main        = get_post_meta($coupon, "superwoo_coupon_coupon_main", true);
				if (!$superwoo_coupon_main) {
					return $price;
				}
				$superwoo_coupon_coupon_type = $superwoo_coupon_main["type"];
				$superwoo_coupon_discounts   = get_post_meta($coupon, "superwoo_coupon_coupon_discounts", true);
				$superwoo_coupon_filters     = get_post_meta($coupon, "superwoo_coupon_filters", true);
				if ($superwoo_coupon_coupon_type == "product") {
					foreach ($superwoo_coupon_filters as $superwoo_coupon_filter) {
						if ($superwoo_coupon_filter["type"] == "products") {
							foreach ($superwoo_coupon_filter["items"] as $superwoo_couponproducts) {
								if ($superwoo_couponproducts["value"] == $product->get_id()) {
									switch ($superwoo_coupon_discounts["type"]) {
										case 'percentage':
											$discount = ($superwoo_coupon_discounts["value"] / 100) * (float)$product->get_regular_price();
											break;
										case 'fixed':
											$discount = $superwoo_coupon_discounts["value"];
											break;
									}
									$amount = ((float)$product->get_regular_price() - $discount);
									$product->set_sale_price($amount);
									return $amount;
								}
							}
						} elseif ($superwoo_coupon_filter["type"] == "all_products") {
							$discount = 0;
							switch ($superwoo_coupon_discounts["type"]) {
								case 'percentage':
									$discount = ($superwoo_coupon_discounts["value"] / 100) * (float)$product->get_regular_price();
									break;
								case 'fixed':
									$discount = $superwoo_coupon_discounts["value"];
									break;
							}
							$amount = ((float)$product->get_regular_price() - $discount);
							$product->set_sale_price($amount);
							return $amount;
						}
					}
				}
			} else {
				return $this->superwoo_coupon_auto_product_coupon($price, $product);
			}
		}
		return $price;
	}

	/*
	 * automatically change product price
	 */
	public function superwoo_coupon_auto_product_coupon($price, $product)
	{
		$data = WC()->session->get("superwoo_coupon_product_coupon");
		foreach ($data["items"] as $woocoupon) {
			$validate = Validator::check(null, null, $woocoupon);
			if (!$validate) {
				return $price;
			}
			$superwoo_coupon_main        = get_post_meta($woocoupon, "superwoo_coupon_coupon_main", true);
			$superwoo_coupon_coupon_type = $superwoo_coupon_main["type"];
			$superwoo_coupon_discounts = get_post_meta($woocoupon, "superwoo_coupon_coupon_discounts", true);
			$superwoo_coupon_filters     = get_post_meta($woocoupon, "superwoo_coupon_filters", true);

			if ($superwoo_coupon_coupon_type == "product") {
				foreach ($superwoo_coupon_filters as $superwoo_coupon_filter) {
					if ($superwoo_coupon_filter["type"] == "products") {
						foreach ($superwoo_coupon_filter["items"] as $superwoo_couponproducts) {
							if ($superwoo_couponproducts["value"] == $product->get_id()) {
								switch ($superwoo_coupon_discounts["type"]) {
									case 'percentage':
										$discount = ($superwoo_coupon_discounts["value"] / 100) * (float)$product->get_regular_price();
										break;
									case 'fixed':
										$discount = $superwoo_coupon_discounts["value"];
										break;
								}
								$amount = ((float)$product->get_regular_price() - $discount);
								$product->set_sale_price($amount);
								return $amount;
							}
						}
						return $price;
					} elseif ($superwoo_coupon_filter["type"] == "all_products") {
						$discount = 0;
						switch ($superwoo_coupon_discounts["type"]) {
							case 'percentage':
								$discount = ($superwoo_coupon_discounts["value"] / 100) * (float)$product->get_regular_price();
								break;
							case 'fixed':
								$discount = $superwoo_coupon_discounts["value"];
								break;
						}
						$amount = ((float)$product->get_regular_price() - $discount);
						$product->set_sale_price($amount);
						return $amount;
					}
				}
			} else {
				return $price;
			}
		}
		return $price;
	}

	public function superwoo_coupon_variable_change_price($price, $product)
	{
		$data = WC()->session->get("superwoo_coupon_product_coupon");
		foreach ($data["items"] as $woocoupon) {
			$validate = Validator::check(null, null, $woocoupon);
			if (!$validate) {
				return $price;
			}
			$superwoo_coupon_main        = get_post_meta($woocoupon, "superwoo_coupon_coupon_main", true);
			$superwoo_coupon_coupon_type = $superwoo_coupon_main["type"];
			$superwoo_coupon_discounts = get_post_meta($woocoupon, "superwoo_coupon_coupon_discounts", true);
			$superwoo_coupon_filters     = get_post_meta($woocoupon, "superwoo_coupon_filters", true);

			if ($superwoo_coupon_coupon_type == "product") {
				foreach ($superwoo_coupon_filters as $superwoo_coupon_filter) {
					if ($superwoo_coupon_filter["type"] == "products") {
						foreach ($superwoo_coupon_filter["items"] as $superwoo_couponproducts) {
							if ($superwoo_couponproducts["value"] == $product->get_parent_id()) {
								switch ($superwoo_coupon_discounts["type"]) {
									case 'percentage':
										$discount = ($superwoo_coupon_discounts["value"] / 100) * (float)$product->get_regular_price();
										break;
									case 'fixed':
										$discount = $superwoo_coupon_discounts["value"];
										break;
								}
								$amount = ((float)$product->get_regular_price() - $discount);
								$product->set_sale_price($amount);
								return $amount;
							}
						}
						return $price;
					} elseif ($superwoo_coupon_filter["type"] == "all_products") {
						$discount = 0;
						switch ($superwoo_coupon_discounts["type"]) {
							case 'percentage':
								$discount = ($superwoo_coupon_discounts["value"] / 100) * (float)$product->get_regular_price();
								break;
							case 'fixed':
								$discount = $superwoo_coupon_discounts["value"];
								break;
						}
						$amount = ((float)$product->get_regular_price() - $discount);
						$product->set_sale_price($amount);
						return $amount;
					}
				}
			} else {
				return $price;
			}
		}
		return $price;
	}
}
